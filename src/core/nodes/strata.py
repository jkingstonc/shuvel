...
class Strata(Node):
    ...
    def __init__(self):
        ...
        self._root_collection=None              # Contains checksum for the root collection this strata is pointing to
    def __str__(self):
    ...
    def checksum_me(self):
        self._checksum=Node.generate_checksum(str(self._creation_date)+str(self._name)+self._root_collection._checksum)
    ...
    def set_root_collection(self, collection):
        self._root_collection=collection

    # Dump the relic contents into a single string
    def get_string_dump(self):
        return str(self._checksum)+Node.INFO_SEPERATOR+str(self._name)+Node.INFO_SEPERATOR+str(self._creation_date)+Node.INFO_SEPERATOR+Node.INFO_CONTENT_SEPERATOR+Node.INFO_SEPERATOR+self._root_collection.get_checksum_short()