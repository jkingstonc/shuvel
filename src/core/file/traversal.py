...
class Traversal:
    ...
    @staticmethod
    def traverse_node(stack, archive_dir):
    ...
    @staticmethod
    def get_level_of_node(root, target, level, archive_dir):
        ...
        if type(root) is Collection:
            for checksum in root._checksums:
                next_node = Load.load_node(checksum,archive_dir)
                result=Traversal.get_level_of_node(next_node,target,level+1,archive_dir)
                if result!=0:
                    return result
        return 0
